...
let isCompactMode = false; // NEW: Compact state tracker
...
function ensureInfoHint() {
  ...
  // NEW: Added dismiss button structure for UX
...
function showInfoHint() {
  ...
  // INCREASED: from 4000 to 8000 for better UX readability
...
// NEW: Performance & Accessibility Upgraded Sidebar Generation
function populateSidebar(islandName, features) {
  ...
  // Performance: using DocumentFragment avoids repainting the browser for every item
  ...
  // Accessibility: Changed header div to an interactive button
  ...
  names.forEach((areaName) => {
    ...
    // Accessibility: Allows 'tabbing' onto areas and clicking with 'Enter'
    item.tabIndex = 0;
    ...
    item.addEventListener("keydown", (e) => { 
      if (e.key === "Enter") window.zoomToArea(islandName, areaName); 
    });
...
window.toggleIsland = (id) => {
  ...
  header.setAttribute("aria-expanded", String(shouldOpen)); // Accessibility sync
...
window.zoomToArea = (islandName, areaName) => {
  setCompactMode(true); // NEW: Compact mode trigger
...
// NEW: Trigger Compact mode on search
